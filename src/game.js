...
import { OPPONENT_PROFILES, createChallengeProgress, recordChallengeEvent, resetChallengeProgress } from './challenges.js';
...
const ui = {
  ...
  opponentName: $('opponentName'), opponentInitial: $('opponentInitial'), profileHeading: $('profileHeading'),
  opponentProfile: $('opponentProfile'), profileDisclosure: $('profileDisclosure'), challengeTitle: $('challengeTitle'),
  challengeDetail: $('challengeDetail'), challengeProgress: $('challengeProgress'), opponentChoice: $('opponentChoice'),
  challengeDock: $('challengeDock')
};

const PROGRESS_KEY = 'the-tell.challenge-progress.v1';
function loadChallengeProgress() {
  try { return createChallengeProgress(JSON.parse(localStorage.getItem(PROGRESS_KEY) || '{}')); }
  catch { return createChallengeProgress(); }
}
...
const state = {
  ...
  opponent: 'vesper', challengeProgress: loadChallengeProgress()
};
...
function currentOpponent() { return OPPONENT_PROFILES[state.opponent]; }
function saveChallengeProgress() {
  try { localStorage.setItem(PROGRESS_KEY, JSON.stringify(state.challengeProgress)); } catch { /* storage is optional */ }
}
function syncOpponentUI() {
  const profile = currentOpponent();
  const progress = state.challengeProgress[profile.id];
  ui.opponentName.textContent = profile.name.toUpperCase();
  ui.opponentInitial.textContent = profile.name[0];
  ui.profileHeading.textContent = `${profile.name.toUpperCase()} PROFILE`;
  ui.opponentProfile.value = profile.id;
  ui.profileDisclosure.textContent = profile.disclosure;
  ui.challengeTitle.textContent = profile.challenge.title;
  ui.challengeDetail.textContent = profile.challenge.detail;
  ui.challengeProgress.textContent = progress >= profile.challenge.target
    ? `COMPLETE · ${progress}/${profile.challenge.target}`
    : `PROGRESS · ${progress}/${profile.challenge.target}`;
}
function markChallenge(event) {
  state.challengeProgress = recordChallengeEvent(state.challengeProgress, state.opponent, event);
...
function setControls(enabled) {
  ...
  ui.decision.textContent = enabled ? 'YOUR DECISION' : (state.handOver ? 'HAND COMPLETE' : `${currentOpponent().name.toUpperCase()} IS THINKING`);
...
function aiProfile() {
  ...
  return { ...currentOpponent().policy, playerAggression, foldRate, exploitBluff: foldRate > .38, trap: playerAggression > .65 };
}
function aiDecision() {
...
async function actAI(epoch = state.handEpoch) {
  ...
  const opponent = currentOpponent().name;
  if (move.type === 'fold') { takeBetAction('ai', { type: 'fold' }); state.stats.aiFolds++; state.log.push(`${opponent} folded`); say(`${opponent} releases the hand. Pressure changed the outcome.`); award('player', `${opponent.toUpperCase()} FOLDS`); return; }
  if (move.type === 'raise') {
    takeBetAction('ai', move); state.stats.aiRaises++;
    state.log.push(`${opponent} raised to ${state.currentBet}`); say(`${opponent} raises to ${chips(state.currentBet)}. Is it strength—or a story?`); shake();
  } else {
    const call = toCall('ai'); takeBetAction('ai', { type: call ? 'call' : 'check' }); state.stats.aiCalls += call > 0 ? 1 : 0;
    state.log.push(call ? `${opponent} called ${call}` : `${opponent} checked`); say(call ? `${opponent} calls. Their range narrows.` : `${opponent} checks. Information, or misdirection?`);
  }
...
async function playerAction(type) {
  ...
  if (type === 'fold' && toCall('player') === 0) return;
  const epoch = state.handEpoch;
  setControls(false);
  if (type === 'fold') {
    takeBetAction('player', { type: 'fold' }); state.stats.playerFolds++; markChallenge('fold-facing-bet'); state.log.push('You folded'); award('ai', 'YOU FOLD'); return;
  }
  if (type === 'check') {
    ...
    state.stats.playerRaises++; markChallenge('pressure-raise'); state.log.push(`You raised to ${state.currentBet}`); say(`You apply pressure. ${currentOpponent().name} must reveal a preference.`); shake();
  }
...
function showdown() {
  ...
  else if (result < 0) award('ai', `${currentOpponent().name.toUpperCase()}'S ${a.name.toUpperCase()} WINS`, { p, a });
  else splitPot(p, a);
}
...
function award(winner, title, hands = null) {
  ...
  const summary = winner === 'player' ? `You collect ${chips(amount)} virtual chips.` : `${currentOpponent().name} collects ${chips(amount)} virtual chips.`;
...
function showReview(title, summary, p, a, note) {
  ...
  ui.reviewGrid.innerHTML = `<div><span>YOUR BEST</span><strong>${p?.name || 'Folded'}</strong></div><div><span>${currentOpponent().name.toUpperCase()} HELD</span><strong>${a?.name || 'Unrevealed'}</strong></div><div><span>STACK CHANGE</span><strong>${formatStackDelta(delta)}</strong></div>`;
...
function profileLabel() {
  return currentOpponent().style.toUpperCase();
}
function updateInspector() {
  ...
  else if (equity < .34 && aiProfile().foldRate > .35) { rec = 'BLUFF SELECTIVELY'; reason = `${currentOpponent().name} has released enough hands for pressure to matter.`; }
...
async function newHand() {
  ...
  state.initialStacks = {...state.stacks}; ui.community.replaceChildren(); ui.street.textContent = 'PRE-FLOP'; renderCards(ui.playerCards, state.player); renderCards(ui.aiCards, state.ai, true); syncOpponentUI();
  ...
  syncBetting(opened, 30); state.log.push(`${state.handDealer === 'player' ? 'You post' : `${currentOpponent().name} posts`} small blind`);
...
$('startBtn').addEventListener('click', () => { $('startOverlay').classList.add('hidden'); ui.challengeDock.append(ui.opponentChoice); state.started = true; $('resetBtn').disabled = false; newHand(); });
...
$('opponentProfile').addEventListener('change', event => { state.opponent = event.target.value; syncOpponentUI(); if (state.started) resetMatch(); });
$('resetProgress').addEventListener('click', () => { state.challengeProgress = resetChallengeProgress(); saveChallengeProgress(); syncOpponentUI(); });
...
syncMotionControl(); syncOpponentUI(); setupAtmosphere(); setControls(false);
