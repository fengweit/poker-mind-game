...
- [ ] P0.4 Run secret-name/value-presence scan on tracked/staged files without printing secrets; `git check-ignore .env research/x/reference-video.mp4`; verify no secrets in git history; rotate exposed credentials when owner is available (owner-only blocker).

## P1 — Accept a complete playable prototype
- [ ] P1.1 Review builder handoff and map actual files in `docs/MANAGER_STATUS.md`. Run `npm test`; record real counts. Commit: `docs: record prototype acceptance evidence`.
- [ ] P1.2 Ranking edge tests in `test/core.test.js`: wheel loses to six-high; full-house chooses higher trips; quads use kicker; board-only ties; flush ranks lexicographically. Run `node --test test/core.test.js`. Use the existing card shape `{rank, suit}`. Commit: `test: cover poker ranking edge cases`.
- [ ] P1.3 Betting contracts in `test/game.test.js` and actual engine path: blinds/order; check prohibited facing a bet; legal call/raise/fold; short all-ins/refunds; repeated terminal action cannot pay twice; reject NaN/negative amounts before mutation. Run `node --test test/game.test.js`; full `npm test`. Commit: `fix: enforce betting and payout invariants`.
